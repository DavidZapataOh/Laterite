...
### Footer
One row on soot, `.on-dark`, padding `clamp(48px, 5.6vw, 100px)` by the gutter: lockup bottom-left; bottom-right, an optional row of mono links over one legal line in lime receipt mono, balanced (`Running on Solana devnet. Tokenized stocks are not available in every country.`). Links come from `lib/site.ts` and render only when they have a real URL; with none, the list is not rendered at all. Stacks left-aligned at 720px and below.
