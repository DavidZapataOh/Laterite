...
| Permission (review) | One permission | the weekly maximum, e.g. $10 with / WK MAX | 0 SOL · Laterite pays fees | none (the foot's Sign once; a Your rules step back above the label) | Unsigned, $N/WK |
...
### Permission ("The permission in numbered clauses")
The band's step back ("Your rules", a drawn arrow) sits above the label; the figure is the weekly maximum and the note, set small (3.4u) so the figure keeps the band, says the wallet pays no SOL. The screen opens at its top. Under the seam, five numbered clauses, sand rules between: a terracotta mono number and one line of Archivo (700, width 104), which never wraps at 390 px, not on Linux either (6 px and 4% of the text left free). The cap clause and the price clause carry their fine print under them in the narrow mono face: who enforces each cap (Laterite's program the combined week, the Subscriptions program each token) and how a buy's minimum is set (fresh, on-chain verified Pyth prices at the careful end of their range, less 0.55%; USDT at its own price, USDC at $1). The clauses keep the comp's rhythm (2.9u above and below each, fine print at 2.9u with a 1.35 line), so on a 390 × 844 phone Sign once, its edge included, is in view with them. Then "What is SPYx?" (or QQQx), a `Disclosure` that opens in place, and one receipt line: the route's test of the exact transaction ("Tested on devnet: it goes through. You pay 0 SOL, Laterite 0.0114 SOL.") before the wallet opens. The foot's Sign once is clay and inert while the test runs and while the wallet or the cluster answers (Approve in {wallet}, Sending to devnet). A refusal is a crimson notice in plain words above the clauses with Try again; a wallet that did not sign leaves Sign once ready.
