...
`DATABASE_URL` references the Postgres, and Railway sets `PORT`. The service refuses to start when a variable is missing or malformed, naming it, when the attestor's key is the crank's, when the treasury's key is not the devnet treasury, and when its RPC's genesis hash is not the one Laterite's config holds.
