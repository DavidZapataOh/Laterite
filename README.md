...
- **Permission and signing:** "Review and sign" opens the permission: the weekly maximum, and what the one signature can and cannot do in five clauses, the caps stated by who enforces them (the combined weekly cap by Laterite's program, each token's plan cap by the Subscriptions program) and how prices are checked. The wallet needs no SOL. `POST /api/sponsor/prepare` builds the onboarding (or, for an exited account, the reactivation) with `@laterite/client`'s builders as ADR-003 shapes it, a version 0 transaction against the onboarding lookup table, sets its compute-unit price from recent fees and its limit from a simulation plus 10%, simulates the exact message and returns it unsigned with what the simulation says; the screen shows that before the wallet opens. The wallet signs once, and `POST /api/sponsor/submit` co-signs as `Config.sponsor` and sends it only when the message is byte for byte one it prepared for that wallet in the last two minutes and never sent, the program is neither paused nor full, and the message is an allowed shape (the sponsor's signature authorizes only its payer's part). Every transaction it builds is recorded in `sponsorships`: three sent a day per wallet, ten per requesting address and 500 in all, two returns per wallet in 30 days, 20 builds an hour per wallet and 60 per address, and the account in an asset, which its owner can close, paid at most once per wallet and asset; every refusal is a JSON line in the function's log.
...
| `SPONSOR_KEYPAIR`            | Server (Vercel) | Secret: `keys/devnet-sponsor.json`'s contents, the key `Config.sponsor` names        |
| `SOLANA_RPC_URL`             | Server (Vercel) | The devnet RPC the faucet and the sponsor send through                               |
...
just app-test                         # build the program and the app, then unit, route and browser tests on local validators (ports 48899 and 49899)
just app-devnet-enrollment            # one real enrollment on devnet by a new wallet with no SOL (needs keys/ and a migrated DATABASE_URL)
