...
export function getPermissionDiscriminatorBytes(): ReadonlyUint8Array {
    return fixEncoderSize(getBytesEncoder(), 8).encode(PERMISSION_DISCRIMINATOR);
}
...
export function getPermissionEncoder(): FixedSizeEncoder<PermissionArgs> {
    return transformEncoder(
        getStructEncoder([
            ...
            ['padding', getArrayEncoder(getU64Encoder(), { size: 30 })],
        ]),
        value => ({ ...value, discriminator: PERMISSION_DISCRIMINATOR }),
    );
}
...
export function getPermissionDecoder(): FixedSizeDecoder<Permission> {
    return getStructDecoder([
        ...
        ['padding', getArrayDecoder(getU64Decoder(), { size: 30 })],
    ]);
}
